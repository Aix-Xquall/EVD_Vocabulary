...
我們想請教一位有航太 Electromagnetic Compatibility (EMC) 經驗的顧問。
...
這是系統級 Electromagnetic Compatibility (EMC) 整合問題。
...
Electromagnetic Compatibility (EMC) 問題常在系統整合階段出現。

**Example 2**

The avionics ring must be designed with Electromagnetic Compatibility (EMC) in mind.

航電環設計時必須考慮 Electromagnetic Compatibility (EMC)。
...
MIL-STD-464 著重於系統級 Electromagnetic Environmental Effects (E3) 要求。

**Example 2**

We need to control Electromagnetic Environmental Effects (E3) risks before vehicle integration.

我們需要在載具整合前控制 Electromagnetic Environmental Effects (E3) 風險。
...
我們需要了解適用的 Electromagnetic Compatibility (EMC) 要求。
...
預掃可在正式測試前找出 Electromagnetic Compatibility (EMC) 問題。

**Example 2**

We should perform pre-scan testing in an Electromagnetic Compatibility (EMC) chamber.

我們應在 Electromagnetic Compatibility (EMC) Chamber 內執行預掃測試。

## 82. Electromagnetic Compatibility (EMC) chamber

**Pronunciation:** /ˌiː.emˈsiː ˈtʃeɪm.bɚ/

**Chinese meaning:** Electromagnetic Compatibility (EMC) 測試艙；電波暗室
...
整合後的航電系統應在 Electromagnetic Compatibility (EMC) 測試艙中測試。
...
我們需要系統級 Electromagnetic Compatibility (EMC) 驗證測試計畫。
...
顧問可支援 Electromagnetic Compatibility (EMC) 設計審查。
...
地迴路是常見的 Electromagnetic Compatibility (EMC) 失效模式。
...
早期測試可以降低 Electromagnetic Compatibility (EMC) 整合風險。
...
導論說明 Electromagnetic Compatibility (EMC) 計畫的目的。
...
本文件範圍涵蓋載具級 Electromagnetic Compatibility (EMC) 風險。
...
許多 Electromagnetic Compatibility (EMC) 問題只有在完整整合後才會浮現。
...
Military Standard 461 (MIL-STD-461) 與 MIL-STD-464 是適用標準。
...
載具級 Electromagnetic Compatibility (EMC) 測試驗證完整系統。
...
適當分配有助於避免超出整體 Electromagnetic Compatibility (EMC) 裕度。
...
任務特定條件需要 Electromagnetic Compatibility (EMC) 裁適。
...
良好的配線設計可降低 Electromagnetic Compatibility (EMC) 風險。
...
電源品質會影響可靠度與 Electromagnetic Compatibility (EMC) 表現。
...
DC-DC 轉換器雜訊是常見 Electromagnetic Compatibility (EMC) 問題。
...
火工品電路需要嚴格 Electromagnetic Compatibility (EMC) 控制。
...
雷擊保護是載具 Electromagnetic Compatibility (EMC) 設計的一部分。
...
## 154. integration-stage verification
...
整合階段驗證確認系統級 Electromagnetic Compatibility (EMC) 行為。
...
良好的測試規劃可降低 Electromagnetic Compatibility (EMC) 鑑定期間的時程風險。

## 156. self-compatibility test
...
Electromagnetic Compatibility (EMC) 稽核會檢查設計與安裝品質。
...
找出雜訊源是 Electromagnetic Compatibility (EMC) 故障排查的第一步。
...
根本原因分析可避免相同 Electromagnetic Compatibility (EMC) 問題再次發生。
...
發射操作會帶來額外 Electromagnetic Compatibility (EMC) 風險。

**Example 2**

Launch operation constraints must be reflected in the Electromagnetic Compatibility (EMC) control plan.

發射操作限制必須反映在 Electromagnetic Compatibility (EMC) 管制計畫中。

## 176. pad RF environment
...
倒數期間必須確認 Electromagnetic Compatibility (EMC) 限制。
...
每項限制條件都應連結到安全或 Electromagnetic Compatibility (EMC) 需求。
...
## 182. Electromagnetic Compatibility (EMC) control plan
...
Electromagnetic Compatibility (EMC) 管制計畫定義設計規則與測試方法。

**Example 2**

The Electromagnetic Compatibility (EMC) control plan should be updated after each major design change.

每次重大設計變更後應更新 Electromagnetic Compatibility (EMC) 管制計畫。
...
附錄提供 Electromagnetic Compatibility (EMC) 報告的佐證資料。
...
## 204. engineering design assumption
...
## 206. subject to final confirmation
...
## 215. Electromagnetic Environmental Effects
...
## 216. system-level Electromagnetic Compatibility (EMC)
...
## 217. Electromagnetic Environmental Effects (E3) Control Plan
...
## 218. mission electromagnetic environment
...
## 219. intra-system Electromagnetic Compatibility (EMC)
...
JetVane operation is a key intra-system Electromagnetic Compatibility (EMC) case for DAQ monitoring.
...
The source-victim matrix should include JetVane, Electronic Power Distribution System (EPDS), RF modules, DAQ, and flight computers.
...
The DAQ module is a likely victim of actuator switching noise.
...
## 238. lightning indirect effects
...
## 245. worst-case simultaneous operation
...
Quiet mode establishes the baseline noise level of DAQ and navigation.
...
DAQ drift and receiver desense are monitored during RF transmit mode.

RF 發射模式期間需監測 DAQ 漂移與接收器 desense。

## 249. actuator worst case
...
The DAQ noise floor is measured before turning on the actuator.
...
## 260. antenna port induced level
...
## 266. intra-system Electromagnetic Compatibility (EMC)
...
## 295. subject to final confirmation
...
## 365. chemical conversion coating
...
## 462. current-carrying capability
...
## 466. controlled interconnection
...
## 689. installation configuration
...
**Category:** Electromagnetic Compatibility (EMC) Behavior / Verification
