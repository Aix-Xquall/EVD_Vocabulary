...
We would like to consult with an experienced aerospace EMC consultant.
...
This is a system-level EMC integration issue.
...
We need to understand the applicable EMC requirements.
...
Harness routing is critical for EMC performance.
...
A pre-scan can identify EMC issues before formal testing.

預掃可在正式測試前找出 EMC 問題。

**Example 2**

We should perform pre-scan testing in an EMC chamber.
...
The integrated avionics system should be tested in an EMC chamber.
...
We need a test plan for system-level EMC verification.
...
The consultant can support the EMC design review.
...
Early testing can reduce EMC integration risk.
...
The introduction explains the purpose of the EMC plan.
...
The scope of this document covers vehicle-level EMC risks.
...
The team should record each EMC issue during integration.
...
Many EMC issues surface only after full integration.
...
The table of contents lists all major EMC topics.
...
EMC tailoring is needed for mission-specific conditions.
...
Poor grounding can increase conducted noise.

不良接地會增加傳導雜訊。

**Example 2**

Grounding design must consider both safety and EMC performance.

接地設計必須同時考慮安全與 EMC 表現。
...
Bonding reduces voltage differences between metal parts.

搭接可降低金屬部件間的電位差。

**Example 2**

All equipment cases should have a controlled bonding path.

所有設備外殼都應有受控的搭接路徑。

## 110. grounding philosophy
...
## 155. integration-stage verification
...
The checklist helps engineers avoid missing critical steps.

檢查表可幫助工程師避免遺漏關鍵步驟。

**Example 2**

The integration checklist should include shield and bonding inspections.
...
Finding the noise source is the first step in EMC troubleshooting.
...
EMC constraints must be checked during countdown.
...
Reducing cable coupling is important for aircraft EMC design.
